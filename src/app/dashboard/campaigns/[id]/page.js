...
import { ArrowLeft, Send, Pause, Play, XCircle, CheckCircle2, Clock, AlertCircle, RefreshCw } from 'lucide-react';
...
export default function CampaignDetailPage({ params }) {
  ...
  const handleAction = async (action) => {
    try {
      ...
      fetchProgress();
    } catch (err) { showToast(`Failed to ${action}`, 'error'); }
  };

  if (loading) return <div className="animate-fade-in"><div className="h-64 rounded-xl bg-surface-800 animate-shimmer" /></div>;
  if (!progress) return <div className="text-center py-20"><p className="text-surface-400">Campaign not found</p></div>;
  ...
  return (
    <div className="animate-fade-in max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex-1"><h1 className="text-2xl font-bold text-surface-100">Campaign Progress</h1></div>
        {isRunning && <button onClick={() => handleAction('pause')} className="btn btn-secondary btn-sm"><Pause className="w-4 h-4" /> Pause</button>}
        {isPaused && <button onClick={() => handleAction('resume')} className="btn btn-primary btn-sm"><Play className="w-4 h-4" /> Resume</button>}
        {(isRunning || isPaused) && <button onClick={() => handleAction('cancel')} className="btn btn-danger btn-sm"><XCircle className="w-4 h-4" /> Cancel</button>}
      </div>

      {/* Status */}
      <div className="card mb-6">
        <div className="flex items-center justify-between mb-4">
          <span className={`badge badge-${progress.status === 'completed' ? 'success' : progress.status === 'running' ? 'info' : progress.status === 'paused' ? 'warning' : progress.status === 'failed' ? 'danger' : 'neutral'}`}>
            {progress.status?.toUpperCase()}
          </span>
          <span className="text-sm text-surface-400">{pct}% complete</span>
        </div>
        <div className="progress-bar mb-6"><div className="progress-bar-fill" style={{ width: `${pct}%` }} /></div>
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            const Icon = s.icon;
            return (<div key={i} className="text-center p-4 rounded-lg bg-surface-800/50">
          ...
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {progress.recentLogs.map((log, i) => (
              <div key={i} className="flex items-center gap-3 p-3 rounded-lg bg-surface-800/50 text-sm">
                {log.status === 'sent' ? <CheckCircle2 className="w-4 h-4 text-success shrink-0" /> : <AlertCircle className="w-4 h-4 text-danger shrink-0" />}
                ...
                <span className={`badge ${log.status === 'sent' ? 'badge-success' : 'badge-danger'} text-[10px]`}>{log.status}</span>
              </div>
            ))}
          </div>
        ) : (
