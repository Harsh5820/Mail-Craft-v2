 ...
 */
export function checkPlanLimits(plan, current) {
  const limits = PLAN_LIMITS[plan] || PLAN_LIMITS.free;
  const violations = [];

  if (limits.dailyEmails !== -1 && current.dailyEmails >= limits.dailyEmails) {
    violations.push(`Daily email limit reached (${limits.dailyEmails})`);
  }
  if (limits.templates !== -1 && current.templates >= limits.templates) {
    violations.push(`Template limit reached (${limits.templates})`);
  }
  if (limits.campaignsPerMonth !== -1 && current.campaignsPerMonth >= limits.campaignsPerMonth) {
    violations.push(`Monthly campaign limit reached (${limits.campaignsPerMonth})`);
  }
